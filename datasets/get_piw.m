...
for data_idx=1:num_data
    ...
    datum.orig_visible = orig_datum.visible;
...
%         if have_frame_skip && ~have_seq_skip
%             fprintf(['Strange frame skip between\n\t%s\nand\n\t%s' ...
%                 '\nin same sequence (%i) (?)\n'], ...
%                 data(data_idx-1).image_path, datum.image_path, seq_num);
%         end
    end
