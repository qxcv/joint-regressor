...
\paragraph{Biposelet centroids} Whenever writing out a positive training sample
for the CNN, the training code also writes out locations of each joint relative
to the crop used to produce the sample. After all positives have been written, a
set of biposelets can be produced for each subpose by clustering those
crop-relative coordinates using $K$-means. The clustering process also yields a
biposelet type for each previously written training sample, which can then be
used as a target label for the CNN\@. Clustering on the same samples used to
train the CNN ensures that the learnt biposelets reflect the range of
augmentations applied to the data, and makes it less likely that some
biposelets will be underrepresented (or not represented at all) in the CNN
training set.
...
process was found to be sufficient to train the model. In contrast to the
Chen et al.~\cite{chen2014articulated}, this work does not use location or type
supervision, as neither were found to improve the learnt weights.
...
results should not be considered representative of the optimal performance of
their approach.
