...
sequence, we obtain a small candidate set of poses for each pair. The task of
the second stage of our pipeline is to obtain a single pose for each frame in
the sequence using those candidate sets. %TODO

Let $\mat p_f$ denote the locations of each joint in the pose chosen for frame
$f \in \{1, \ldots, F\}$, and $C(\mat p_f)$ denote the cost associated with the
subpose %TODO

For a sequence of $F$ frames, we %TODO
$(\mathcal P_{n-1,n}, \mathcal P_{n,n+1})$

\begin{equation}\label{eqn:stitch-cost}
\sum_{f=2}^{F-1} \|\mat p_{f-1} - \mat p_f\|_2^2 
+ \sum_{f=1}^{F-1} C(\mat L_f, \vec t_f \mid \mat D_{f,f+1})
\end{equation}
