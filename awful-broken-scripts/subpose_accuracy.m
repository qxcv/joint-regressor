function subpose_accuracy(cy_pred_path)
%SUBPOSE_ACCURACY How  good are biposelets at predicting subpose locations?
% Will extract ground truth locations for a particular subpose in a frame
% pair, then run the biposelet classifier to find locations for joints
% within that subpose. Locations are translated back into original image
% coordinates and saved, along with the ground truth locations, to a file.
%
% This should allow me to make a comparison with Chen & Yuille's
% predictions.

% Start by loading everything we need, so that errors are caught early.
fprintf('Loading configuration and model\n');
startup;
conf = get_conf_mpii;
try
    [cy_preds, test_seqs] = ...
...
% XXX: to_pair_dets needs to be changed
pair_dets = to_pair_dets(test_seqs, ssvm_model, conf.subposes, ...
    biposelets, conf.cache_dir);

% XXX: Everything from here down can be removed
...
function pair_dets = to_pair_dets(test_seqs, ssvm_model, ...
...
for seq_idx=1:num_seqs
    ...
    for fst_idx=1:num_pairs
        ...
        fst_gt = d1.joint_locs;
        snd_gt = d2.joint_locs;
        true_scale = calc_pair_scale(fst_gt, snd_gt, subposes, ...
            ssvm_model.template_scale);
        % XXX: The score_pairs thing is not applicable here.
