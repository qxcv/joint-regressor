...
if size(out, 1) == 0 && size(out, 2) == 0
    ...
    if bl < 1
        % Left-pad
        out = padarray(out, [0, min(1 - bl, bw)], 'replicate', 'pre');
    end
    if bt < 1
        % Top-pad
        out = padarray(out, [min(1 - bt, bh), 0], 'replicate', 'pre');
    end
    if box(1) + bw > imw
        % Right-pad
        out = padarray(out, [0, min(box(1) + bw - imw, bw)], 'replicate', 'post');
    end
    if box(2) + bh > imh
        % Bottom-pad
        out = padarray(out, [min(box(2) + bh - imh, bh), 0], 'replicate', 'post');
    end
end
